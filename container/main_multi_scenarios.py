...
def fim_multiple_scenarios(dam_id, input_dir, temp_path):
    ...
    fim_path_mh = f"{input_dir}/{sce_mh['loadCondition']}_{sce_mh['breachCondition']}_{dam_id}.tiff"
    ...
    fim_path_tas = f"{input_dir}/{sce_tas['loadCondition']}_{sce_tas['breachCondition']}_{dam_id}.tiff"
    ...
    fim_path_nh = f"{input_dir}/{sce_nh['loadCondition']}_{sce_nh['breachCondition']}_{dam_id}.tiff"
...
def extract_fim_geoid(dam_id, input_dir, temp_path, tract_gdf):
    ...
    input_url = "https://www2.census.gov/geo/tiger/TIGER2020/TABBLOCK20/"
    out = subprocess.run(f"curl -o /tmp/tl_2020_{fim_state[0]}_tabblock20.zip {input_url}/tl_2020_{fim_state[0]}_tabblock20.zip", shell=True)
    out = subprocess.run(f"cd /tmp; unzip /tmp/tl_2020_{fim_state[0]}_tabblock20.zip", shell=True)
    input_dir1 = f"/tmp/"
    if len(fim_state) == 1: # If only one state is associated with the inundation mapping
        block_gdf = gpd.read_file(f'{input_dir1}/tl_2020_{fim_state[0]}_tabblock20.shp')
        ...
        for state_num in fim_state:
            temp_gdf = gpd.read_file(f'{input_dir1}/tl_2020_{state_num}_tabblock20.shp')
    ...
    for water_cls in fim_gdf['value'].unique():
        ...
        for geoid_ in fim_geoid_['GEOID20'].to_list():
    ...
    fim_geoid_gdf = fim_geoid_df.merge(block_gdf, on='GEOID20')
    ...
    fim_geoid_gdf = fim_geoid_gdf.groupby(['Dam_ID', 'GEOID20'], 
...
def calculate_bivariate_Moran_I_and_LISA(dam_id, census_dic, fim_geoid_gdf, dams_gdf):
    ...
    input_cols.extend(['Dam_ID', 'GEOID20', 'Class', 'geometry'])
    ...
    for census_name in census_dic.keys():
        ...
        ['Dam_ID', 'GEOID20', 'Class', census_name, 'geometry']].reset_index(drop=True)
        ...
        fim_geoid_local_na = fim_geoid_local.loc[fim_geoid_local[census_name].isna(), ['Dam_ID', 'GEOID20', 'Class', census_name, 'geometry']]
        ...
        fim_geoid_local = fim_geoid_local.merge(fim_geoid_local_var[['GEOID20', f'LISA_{new_col_name}']], on='GEOID20')
...
def spatial_correlation(dam_id, dams_gdf, fim_geoid_gdf, census_dic, census_df):
    ...
    fim_geoid_gdf['GEOID_T'] = fim_geoid_gdf.apply(lambda x:x['GEOID20'][0:11], axis=1)
...
if __name__ == "__main__":
    ...
    fed_dams = pd.read_csv('https://raw.githubusercontent.com/cybergis/population_vulnerable_to_dam_failure_compute/refs/heads/main/dam_list.csv')
    ...
    census_data = pd.read_csv("https://raw.githubusercontent.com/I-GUIDE/population_vulnerable_to_dam_failure/refs/heads/main/sample_data/census_geometry/census_data.csv")
    ...
    print(f"Saving results Multi_F_fim.geojson")
    #fim_output.to_file(os.path.join(output_dir, f"Multi_F_fim.geojson"), driver='GeoJSON')
    fim_output.to_file("Multi_F_fim.geojson", driver='GeoJSON')
    print(f"Saving results Multi_F_mi.geojson")
    #mi_result.to_file(os.path.join(output_dir, f"Multi_F_mi.geojson"), driver='GeoJSON')
    mi_result.to_file("Multi_F_mi.geojson", driver='GeoJSON')
    print(f"Saving results Multi_F_lm.geojson")
    #lm_result.to_file(os.path.join(output_dir, f"Multi_F_lm.geojson"), driver='GeoJSON')
    lm_result.to_file("Multi_F_lm.geojson", driver='GeoJSON')
